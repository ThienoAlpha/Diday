...
<body>
	...
	<%
		String ten_tai_khoan_err="",ten_nguoi_dang_nhap_err="",mat_khau_err="";
		...
		if(request.getAttribute("ten_dang_nhap_err") != null){
			ten_nguoi_dang_nhap_err = (String)request.getAttribute("ten_dang_nhap_err");
		}
		if(request.getAttribute("mat_khau_err") != null){
			mat_khau_err = (String)request.getAttribute("mat_khau_err");
		}
		String ten_tai_khoan="",ten_nguoi_dang_nhap="",mat_khau="";
		if(request.getAttribute("ten_tai_khoan") != null){
			ten_tai_khoan = (String)request.getAttribute("ten_tai_khoan");
		}
		if(request.getAttribute("ten_nguoi_dang_nhap") != null){
			ten_nguoi_dang_nhap = (String)request.getAttribute("ten_nguoi_dang_nhap");	
		}
		if(request.getAttribute("mat_khau") != null){
			mat_khau = (String)request.getAttribute("mat_khau");
		}
		String success = "";
		if(ten_nguoi_dang_nhap_err.length()==0 && ten_tai_khoan_err.length()==0 && mat_khau_err.length()==0 && ten_nguoi_dang_nhap.length()>0 && ten_tai_khoan.length()>0 && mat_khau.length()>0) {
			...
			<div class="row">
				<div class="col-sm-4 col-sm-offset-1">
					<div class="login-form">
						<!--login form-->
						<h2>Đăng nhập</h2>
						<form action="DangNhapServlet?case=login" method="post">
							<input type="email" placeholder="Email" name="ten_tai_khoan_dn" />
							<input type="password" placeholder="Mật khẩu" name="mat_khau_dn" />
							<p style="color:red"><%=dang_nhap_err%></p> <span> 
							<input type="checkbox" class="checkbox"> Duy trì đăng nhập
							</span>
					...
					<div class="login-form">
						<!--login form-->
						<form style="margin-top:-15px" action="DangNhapServlet?case=guest" method="post">
							<input type="hidden" name="id_Guest" value="<%=System.currentTimeMillis()%>" />
							<button type="submit" class="btn btn-default">Đăng nhập với tư cách khách</button>
						</form>
					</div>
				...
				<div class="col-sm-4">
					<div class="signup-form">
						...
						<form action="DangKyServlet" method="post" >
							<p style="color:red"><%=ten_nguoi_dang_nhap_err%></p>
							<input type="text" placeholder="Họ và tên" name="ten_nguoi_dang_nhap" value ="<%=ten_nguoi_dang_nhap%>"/>
							<p style="color:red"><%=ten_tai_khoan_err%></p>
							<input type="email" placeholder="Email đăng ký" name = "ten_tai_khoan" value="<%=ten_tai_khoan%>"/> 
